...
EXISTENT_COLORS = [
    ...
    "rose quartz 2.0", "stone", "tigerlily", "tigerlily plum"
]
...
def scrape_product_details(driver, url, is_first):
    try:
        ...
        driver.get(url)
        time.sleep(10 if is_first else 3)  # Wait for 10 seconds for the first page, 3 seconds for others
    ...
    try:
        product_color = soup.find('div', id='variation_color_name').find('span', class_='selection').text.strip()
    ...
    try:
        ...
        asin2 = product_details_table.find('td', class_='a-size-base prodDetAttrValue').text.strip()
    except Exception as e:
        asin2 = 'N/A'
        print(f"Error extracting ASIN (second method): {e}")

    try:
        product_color2 = product_details_table.find('td', class_='a-size-base prodDetAttrValue').text.strip()
    ...
    try:
        product_color3 = soup.find('span', class_='a-size-base po-break-word').text.strip()
    ...
    try:
        ...
        for li in ul.find_elements(By.TAG_NAME, 'li'):
            ...
            dp_url = li.get_attribute('data-dp-url')
            asin_from_url = extract_asin_from_url(dp_url)

            if asin_id == asin_from_url:
                asin_list.append(asin_id)
            else:
                asin_list.append(asin_id)
                asin_list.append(asin_from_url)
    except Exception as e:
        asin_list = []
...
if __name__ == "__main__":
    ...
    csv_path = '/Users/wavesmanagement/scripts/urls.csv'  # Path to the CSV file
