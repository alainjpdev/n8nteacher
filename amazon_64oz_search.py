...
def toggle_pause():
    ...
    paused = not paused
    if paused:
        print("Script paused. Press Space to continue...")
    else:
...
# Functions to extract information from the title, URL, and page content are assumed to be already defined above
...
def scrape_product_details(driver, url, is_first):
    ...
    try:
        ...
        driver.get(url)
        time.sleep(10 if is_first else 3)  # Wait 10 seconds if it's the first attempt, otherwise 3 seconds
        print("Page accessed successfully.")
    except Exception as e:
        print("Error accessing the URL:", e)
        print("Retrying URL after a 10-second wait...")
        time.sleep(10)  # Wait 10 seconds before retrying
        try:
            driver.get(url)
            time.sleep(10)  # Wait 10 seconds again after the retry attempt
            print("Page accessed successfully on retry.")
        except Exception as retry_e:
            print("Failed to retry accessing the URL:", retry_e)
            return []
...
if __name__ == "__main__":
    ...
    # Load ASINs from the new CSV file
    asin_path = '/Users/wavesmanagement/scripts/amazonasin64oz.csv'  # Path to the CSV file with ASINs
    df_asin = pd.read_csv(asin_path)
    asins = df_asin['ASIN'].tolist()  # Assuming the column name is 'ASIN'

    # Construct URLs based on ASINs
    base_url = "https://www.amazon.ca/dp/"
    urls = [f"{base_url}{asin}" for asin in asins]
